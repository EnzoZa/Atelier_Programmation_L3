...
def date_est_valide(day,month,year) :
    ...
    if Exercice_2.est_bissextile(year) and month == FEBRUARY:
        ...
        if (not (month % 2) and month != JULY) and (day > MAX_DAY_ODD or day < MIN_GENERAL): #On verifie la validité du nombre du jour rentré pour un mois de 30 jours
            res = False
        elif ((month % 2) and month != JULY) and (day > MAX_DAY_EVEN or day < MIN_GENERAL) : #On verifie la validité du nombre du jour rentré pour un mois de 31 jours
            res = False
        else: 
            res = True
    return res
...
def est_majeur(birthdate :datetime.date) :
    VALUE_MAJORITE = 18
    #Tout s'implement si l'age est au dessus de la constante fixer pour l'age (18ans en france sinon ça pourrait être 21ans au étéat UNIS) alors return True
